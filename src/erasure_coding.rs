...
use super::{ReedSolomon, wasm_bindgen, JsValue, to_value, Uint8Array};
...
pub fn encode_shards(data: Vec<u8>, data_shards: usize, parity_shards: usize) -> Result<JsValue, JsValue> {
...
pub fn reconstruct_shards(shards: Vec<JsValue>, data_shards: usize, parity_shards: usize) -> Result<JsValue, JsValue> {
    let reed_solomon = ReedSolomon::new(data_shards, parity_shards).unwrap();
    let mut shards: Vec<Option<Vec<u8>>> = shards.into_iter().map(|js_value| {
    ...
    reed_solomon.reconstruct(&mut shards).map_err(|_| <ErasureError as Into<JsValue>>::into(ErasureError::FragmentationError))?;
