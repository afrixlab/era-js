...
async function main() {
	...
	let signer = new Signer(polkadot_signer.fetch_key().private_key);
	console.log(signer.getPolkadotSigner().publicKey);
...
class EraSDK {
	...
	async getWallet(
		...
		try {
			...
			let data: GetWalletResponse = (await response).data;
			return data;
		} catch (error) {
			console.log(error);
			throw error;
